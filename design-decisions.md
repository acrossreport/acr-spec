...
5. [ADR-003: Adopt a Section-Based Report Structure](#adr-003-adopt-a-section-based-report-structure)
...
| ADR-003 | Adopt a Section-Based Report Structure | Accepted |
...
The example below follows the format defined in [specification.md](specification.md) (A4 at 300 DPI, in dots).
...
## ADR-003: Adopt a Section-Based Report Structure

### Context

The question was whether to use the section-based structure widely adopted in business reporting (ReportHeader / PageHeader / GroupHeader / Detail / GroupFooter / PageFooter / ReportFooter) as the reference design for ACR's layout model.

### Decision

**Adopt the section-based report structure as the standard layout model for ACR.**

This does not prevent ACR-specific extensions; the section structure is the *design starting point*, not a hard constraint.

### Rationale

1. **Leveraging existing knowledge:** Many field engineers already understand the section-based model, minimizing learning costs.
2. **Reduced migration cost:** Existing section-based report definitions can be incrementally migrated to ACR.
3. **Battle-tested design:** The section-based structure has been validated over decades of business report processing and reflects operational needs at a high level.
...
| Benefits | Drawbacks |
|----------|-----------|
| No additional learning required for field engineers | Cost of maintaining compatibility with existing report definitions |
| Easy migration of existing report assets | Some ACR-specific design choices may be constrained |
| Inherits proven design philosophy | Risk of inheriting limitations of conventional section-based designs |
...
4. **Core technical differentiator (patent pending):** "Pixel-perfect preview without physical hardware" is the core technical advantage of ACR. The intermediate drawing model that makes it possible is patent pending.
...
| 0.2 | 2026-09-23 | Revised ADR-003 as a section-based structure; aligned the ADR-002 example with specification.md; updated ADR-005 wording (patent pending) | MaskedRiderSystem |
